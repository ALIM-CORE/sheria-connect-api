package co.tz.sheriaconnectapi.exceptions;

import lombok.Getter;

@Getter
public enum ErrorMessages {
    MEMBER_NOT_FOUND("Member not found"),
    ROLE_NOT_FOUND("Role not found"),
    AUTHORITY_NOT_FOUND("Authority not found"),
    EMAIL_ALREADY_EXISTS("Email already exists"),
    INVALID_CLIENT_TYPE("Invalid client type"),
    INVALID_REQUEST_PAYLOAD("Invalid request payload"),
    ACCESS_DENIED("Access denied"),
    AUTHENTICATION_REQUIRED("Authentication is required"),
    UNEXPECTED_ERROR("An unexpected error occurred. Please try again later."),
    USER_NOT_VALID("Invalid request"),
    ACCESS_MANAGEMENT_ERROR("Access management request could not be completed"),
    INVALID_TOKEN("Invalid token"),
    MISSING_TOKEN("Missing refresh token"),
    EMAIL_ALREADY_VERIFIED("Email already verified"),
    EMAIL_NOT_VERIFIED("This email has not been verified yet"),
    EMAIL_DELIVERY_FAILED("Email could not be sent right now. Please try again later."),
    EMAIL_VERIFICATION_TOKEN_EXPIRED("Email verification token expired"),
    EMAIL_VERIFICATION_TOKEN_INVALID("Invalid email verification token"),
    PASSWORD_RESET_TOKEN_INVALID("Invalid password reset token"),
    PASSWORD_RESET_TOKEN_EXPIRED("Password reset token expired"),
    INVALID_LOGIN_CREDENTIALS("Wrong email or password"),
    WEB_PORTAL_ACCESS_DENIED("This account is not allowed to access the owner web portal"),
    INCIDENT_REPORT_NOT_FOUND("Incident report not found"),
    INVALID_CASE_NUMBER("Invalid case number"),
    INVALID_TRACKING_TOKEN("Invalid tracking token"),
    UNAUTHORIZED_CASE_ACCESS("You are not allowed to access this case"),
    STAFF_SELF_REVIEW_DENIED("Staff cannot review reports they submitted from their citizen account."),
    MISSING_EVIDENCE_FILE("Evidence file is required"),
    EVIDENCE_FILE_NOT_FOUND("Evidence file not found"),
    UNSUPPORTED_EVIDENCE_TYPE("Unsupported evidence file type"),
    EVIDENCE_FILE_TOO_LARGE("Evidence file is too large"),
    EVIDENCE_STORAGE_FAILED("Evidence file could not be stored"),
    INVALID_STATUS_TRANSITION("Invalid case status transition"),
    INCIDENT_CATEGORY_NOT_FOUND("Incident category not found"),
    INVALID_INCIDENT_CATEGORY("Incident category is invalid or unavailable"),
    DUPLICATE_INCIDENT_CATEGORY("Incident category already exists"),
    PROVIDER_PROFILE_NOT_FOUND("Provider profile not found"),
    UNAUTHORIZED_PROVIDER_PROFILE_ACCESS("You are not allowed to access this provider profile"),
    MATCHING_REQUEST_NOT_FOUND("Matching request not found"),
    INVALID_MATCHING_REQUEST_STATUS("Invalid matching request status"),
    DUPLICATE_MATCHING_REQUEST("This provider has already been matched to the case"),
    NOTIFICATION_NOT_FOUND("Notification not found"),
    NOTIFICATION_ACCESS_DENIED("You are not allowed to access this notification"),
    KNOWLEDGE_ARTICLE_NOT_FOUND("Knowledge article not found"),
    CASE_MESSAGE_NOT_ALLOWED("You are not allowed to access messages for this case"),
    STORY_NOT_FOUND("Story not found"),
    UNAUTHORIZED_STORY_ACCESS("You are not allowed to access this story"),
    STORY_NOT_PUBLISHED("This story is not published"),
    INVALID_STORY_MODERATION_STATUS("Invalid story moderation status"),
    INVALID_STORY_CONTENT("Story content is invalid"),
    ;

    private final String message;

    ErrorMessages(String message) {
        this.message = message;
    }

}
